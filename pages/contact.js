...
export default function Contact() {
    const { setState, bindings } = useTabs(null)
    return <>
    <NavBar />
        <Page style={{ fontFamily: "var(--normalFont) "}}>
            <Text h1>Contact Us</Text>
            {
                bindings.value == null && <Grid justify="center" alignItems="center" style={{ display: "flex", height: "50vh", flexDirection: "column" }}>
                    <Text h2 style={{ fontFamily: "var(--normalFont)" }}>I want:</Text>
                    <ButtonGroup>
                        <Button onClick={() => setState("website")}>a website/app</Button>
                        <Button onClick={() => setState("information")}>Some information</Button>
                    </ButtonGroup>
                </Grid>
            }
            {
                bindings.value != null && <Tabs {...bindings}>
                    <Tabs.Item label="Website" value="website">
                        <Spacer y={1} />
